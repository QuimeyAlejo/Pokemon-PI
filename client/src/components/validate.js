export default function validate(input, pokemons){
    ...
    // if(pokemons.indexOf( input.name ) !== -1){  errors.name = 'A pokemon with that name is already existing' }
    if(!RegExpression.test(input.name)){errors.name = 'Numbers or special characters are not allowed'}   
    if(input.name.length > 18){errors.name = `The name can't be longer than 18 characters`}
